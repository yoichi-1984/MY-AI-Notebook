...
4. **ページ単位のToDoリスト（タスク管理）機能 🌟**
   - ページごとにタスクを追加・管理できる「ToDoリスト」タブを実装しました。
   - ノートを書きながらでも、素早くタスク内容・期日・優先度を登録できます。
   - チェックリスト形式で進捗管理ができ、タスクの状態は即座にデータベースに同期されます。
...
8. **ハイブリッド検索 ＆ ローカルRAG基盤（マルチベクトル構成）**
   - **マルチベクトル検索**: これまでの単一ベクトルから進化し、「本文」「AI要約」「AIタグ」それぞれ独立した512次元のベクトル空間を構築。LanceDBの近傍検索（ANN）で多角的に意味の近いページを抽出します。（複数ページがヒットした際は、RRFスコアの最も高い1ページに代表させて重複を排除します）
   - **キーワード検索（全文検索）**: 全画像のOCRテキスト、添付ファイル抽出テキストに加え、ノートの「タイトル」や「ページ名」に対してもLanceDBのFTS（Full-Text Search）を利用し、キーワード完全一致でヒットさせます。
   - **RRF 融合 ＆ ローカル RAG**: ベクトル検索3系統と全文検索の結果を RRF (Reciprocal Rank Fusion) で統合スコアリング。ヒットした上位コンテキストを Gemini に流し込み、過去のナレッジを100%の根拠とした「調査レポート」を生成します。
...
9. **ダークモード ＆ AIパラメータ設定機能**
   - OneNote風の3カラムUIは、ライト/ダークテーマの切り替えに対応し、フォルダやノートのドラッグ＆ドロップによる自由な並び替え（ソート）も可能です。
   - 設定画面から「AIモデル」「推論レベル（`thinking_level`）」「自動仕分け閾値」「RAGパラメータ」などを動的に変更し、SQLiteに保存・反映できます。

10. **完全なオフライン耐性 ＆ ジョブキューイング 🌟**
    - ネットワーク不通時やAPIエラー発生時でもUIはフリーズせずに入力を受け付けます。背後でAI処理ジョブを SQLite の `api_jobs` テーブルにキューイングし、オンライン復帰時にバッチエンジンが自動で再処理を再開します。
    - オフライン状態は画面上のインジケーターでリアルタイムに通知され、処理待ちのノートには砂時計バッジが表示されます。

11. **データ保存先の動的変更（ポータビリティ）🌟**
    - 設定画面から、データベース（SQLite/LanceDB）や物理ファイル（画像・添付ファイル）の保存先パスを外部ドライブ等へ自由に指定可能です。
    - サーバーの再起動を伴わずにバックエンドで即座にパスが切り替わり、既存データの自動移行もシームレスに実行されます。
...
graph TD
    ...
    subgraph ExternalAPI ["外部 AI サービス"]
        ...
        OpenAI["Azure OpenAI Service<br/>- text-embedding-3-small (512次元マルチベクトル化)"]
    end
...
- **利用AI**:
  ...
  - **ベクトル化 (Embedding)**: Azure OpenAI `text-embedding-3-small` (512次元マルチベクトル)
...
- **`tasks`**: ページに紐づくTODOタスク（完了日時 `completed_at` や優先度 `priority` の管理に対応）。
- **`settings`**: AIやUIの各種パラメータ設定（デフォルトのタスク優先度なども含む）。
- **`api_jobs`**: オフライン時等に失敗したAI処理を一時退避するジョブキュー。

### 2. 自動マイグレーション機能
データベース接続初期化時 (`sqlite_client.init_db()`) に、`PRAGMA user_version` を利用した段階的マイグレーションが自動実行されます。旧バージョン（シングル画像仕様、シングルページ仕様など）のデータベースが存在する場合でも、データ損失なしで最新のバージョン 10 スキーマへと安全にアップデートされます。
...
- `PUT /api/folders/reorder`: フォルダの順序変更
- `PUT /api/folders/{folder_id}/notes/reorder`: フォルダ内ノートの順序変更
...
- `GET /api/settings`: 現在のAI・UI設定・保存先パス一覧の取得
- `POST /api/settings`: 設定の更新（SQLiteへの保存・即時反映）
- `POST /api/settings/path`: データベース・ファイルの保存先パスの動的変更とデータ移行
